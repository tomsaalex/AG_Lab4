...
int main(int argc, char** argv)
{
	...
	for (int i = 1; i <= alphabetLength; i++)
	{
		...
		fin >> letterFrequency;
		fin.get();
		alphabetMap.insert(make_pair(letter, letterFrequency));
	}
	...
	string codedSequence;
	string decodedString;
	Nod* root = nodesVector[0];
	Nod* lastChecked = root;


	for (char codeChar : codedString)
	{

		if (lastChecked->data != NULL)
		{
			decodedString += lastChecked->data;
			lastChecked = root;
		}


		if (codeChar == '0')
			lastChecked = lastChecked->st;
		else if (codeChar == '1')
			lastChecked = lastChecked->dr;
	}
